...
def UCT_search(game_state, num_reads, net, device='cuda'):
    ...
    for idxx in range(0,num_games):
        ...
        while checkmate == False and current_board.move_count <= 100:
            ...
            if current_board.move_count % 10 == 0:
                print(current_board.current_board, flush=True)
...
def MCTS_self_play(chessnet, num_games, cpu, device='cuda'):
    for idxx in range(0, num_games):
        current_board = c_board()
        checkmate = False
        dataset = []
        ...
        try:
            while checkmate == False and current_board.move_count <= 100:
                ...
                states.append(copy.deepcopy(current_board.current_board))
                board_state = copy.deepcopy(ed.encode_board(current_board))
                best_move, root = UCT_search(current_board, 777, chessnet, device)
                current_board = do_decode_n_move_pieces(current_board, best_move)  # decode move and move piece(s)
                policy = get_policy(root)
                dataset.append([board_state, policy])
                print(f"Process {cpu}, Game {idxx}, Move {current_board.move_count}")
                if current_board.check_status() == True and current_board.in_check_possible_moves() == []:  # checkmate
                    if current_board.player == 0:  # black wins
                        value = -1
                    elif current_board.player == 1:  # white wins
            ...
            save_as_pickle("dataset_cpu%i_%i_%s" % (cpu, idxx, datetime.datetime.today().strftime("%Y-%m-%d")), dataset_p)
            print(f"Process {cpu}, Game {idxx} completed and saved")
            
        except Exception as e:
            print(f"Error in Process {cpu}, Game {idxx}: {e}")

def run_self_play(net, num_games=10, num_processes=1):
    try:
        ...
        # Share memory if using multiprocessing
        if num_processes > 1:
            net.share_memory()
        ...
        for i in range(num_processes):
            p = mp.Process(target=MCTS_self_play, args=(net, num_games, i, device))
            p.start()
            processes.append(p)
        ...
        print("All self-play processes completed")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # Run self-play
    num_processes = min(6, os.cpu_count() or 1)
    run_self_play(net, num_games=10, num_processes=num_processes)
